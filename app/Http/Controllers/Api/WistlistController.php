...
class WistlistController extends Controller
{
	public function create_wistlist(Request $request)
	{
		...
		if(Auth::guard('sanctum')->check()) 
		{
			...
			foreach ($request->contacts as $contact_val) {
				if($contact_val['email'] != null){
					...
					$retailer_id =  null;
					$exsits = User::where('email',$contact_val['email'])->first();
					if(isset($exsits))
					{
					...
					$notification->wishlist_email = $contact_val['email'] ?? null;
					...
					$data = [
						...
						'toEmails' => array($contact_val['email']),
