	...
	<div class="content container-fluid">
		...
		<div class="page-header">
			<div class="row align-items-center">
				<div class="col-md-4">
					<h3 class="page-title">{{ __('artist') }}</h3>
					<ul class="breadcrumb">
						<li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ __('dashboard') }}</a></li>
						<li class="breadcrumb-item active">{{ __('artist') }}</li>
					</ul>
				</div>
				<div class="col-md-8 float-end ms-auto">
					<div class="d-flex title-head">
						...
						<a href="#" class="btn add-btn add_artist" data-bs-toggle="modal" data-bs-target="#add_artist"><i class="la la-plus-circle"></i> {{ __('add_artist') }}</a>
					</div>
				</div>
			</div>
		</div>
		...
		<form name="search-frm" method="post" action="{{ route('user.artist')}}" id="search-product-code-frm">
		@csrf
			<div class="row filter-row">
				<div class="col-xl-3">  
					 <div class="input-block">
						 <input type="text" class="form-control floating" name="search_name" placeholder="{{ __('artist')}}">
					 </div>
				</div>
				<div class="col-xl-3">  
					 <div class="input-block">
							...
							<tr>
								...
								@if($val->status ==1)
									<div class="dropdown action-label">
										...
										<div class="dropdown-menu dropdown-menu-right">
											<a class="dropdown-item update-status" href="javascript:void(0);" data-id="{{ $val->id }}" data-url="{{ route('artist-update-status') }}"><i class="fa-regular fa-circle-dot text-success"></i> {{ __('active') }}</a>
											<a class="dropdown-item update-status" href="javascript:void(0);" data-id="{{ $val->id }}" data-url="{{ route('artist-update-status') }}"><i class="fa-regular fa-circle-dot text-danger"></i> {{ __('inactive') }}</a>
										</div>
									</div>
								 @else
									<div class="dropdown action-label">
								...
								<td class="text-end">
									<div class="dropdown dropdown-action">
										...
										<div class="dropdown-menu dropdown-menu-right">
											<a class="dropdown-item edit-artist" href="javascript:void(0);" data-id="{{ $val->id ??''}}" data-url="{{ route('edit-artist') }}"><i class="fa-solid fa-pencil m-r-5"></i> {{ __('edit') }}</a>
