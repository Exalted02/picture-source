	...
	<div class="content container-fluid">
		...
		<div class="page-header">
			<div class="row align-items-center">
				<div class="col-md-4">
					<h3 class="page-title">{{ __('size') }}</h3>
					<ul class="breadcrumb">
							...
							<tr>
								...
								@if($val->status ==1)
									<div class="dropdown action-label">
										...
										<div class="dropdown-menu dropdown-menu-right">
											<a class="dropdown-item update-status" href="javascript:void(0);" data-id="{{ $val->id }}" data-url="{{ route('size-update-status') }}"><i class="fa-regular fa-circle-dot text-success"></i> {{ __('active') }}</a>
											<a class="dropdown-item update-status" href="javascript:void(0);" data-id="{{ $val->id }}" data-url="{{ route('size-update-status') }}"><i class="fa-regular fa-circle-dot text-danger"></i> {{ __('inactive') }}</a>
										</div>
									</div>
								 @else
									<div class="dropdown action-label">
								...
								<td class="text-end">
									<div class="dropdown dropdown-action">
										...
										<div class="dropdown-menu dropdown-menu-right">
											<a class="dropdown-item edit-size" href="javascript:void(0);" data-id="{{ $val->id ??''}}" data-url="{{ route('edit-size') }}"><i class="fa-solid fa-pencil m-r-5"></i> {{ __('edit') }}</a>
											<a class="dropdown-item delete-size" href="javascript:void(0);" data-id="{{ $val->id ?? '' }}" data-url="{{ route('getDeleteSize') }}"><i class="fa-regular fa-trash-can m-r-5"></i> {{ __('delete') }}</a>
										</div>
									</div>
								</td>
							</tr>
...
<script src="{{ url('front-assets/js/page/size.js') }}"></script>
